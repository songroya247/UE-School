...
const QUESTION_BANK = (function () {
  ...
  // Async version: tries GSheet first, falls back to local bank
  async function getTopicsAsync(subject) {
    if (window.GSHEET_QUESTIONS && window.GSHEET_QUESTIONS.isEnabled()) {
      try {
        const rows = await window.GSHEET_QUESTIONS.getQuestions({ subject, count: 200 });
        if (rows && rows.length) {
          const sheetTopics = [...new Set(rows.map(q => q.topic).filter(Boolean))].sort();
          if (sheetTopics.length) return sheetTopics;
        }
  ...
  return { getQuestions, getMockExam, getSubjects, getTopics, getTopicsAsync, countFor };
