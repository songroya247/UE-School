...
const CLASSROOM = (function () {
  ...
  // ─── Render lesson ────────────────────────────────
  async function renderLesson(topic) {
    // Title + meta
    ...
    if (videoArea) {
      ...
      // 1. Get student's mastery level for this topic from Supabase
      //    (null = brand new student → defaults to foundation tier)
      let masteryScore = null;
      if (typeof userId !== 'undefined' && userId && window.sb) {
        try {
          const { data } = await window.sb
            ...
            .maybeSingle();
          masteryScore = data?.mastery_level ?? null;
      ...
      const tier     = window.GDRIVE_CURRICULUM
                       ? GDRIVE_CURRICULUM.masteryToTier(masteryScore)
                       : 'standard';
      const resolved = window.GDRIVE_CURRICULUM
                       ? GDRIVE_CURRICULUM.resolveVideo(topicForVideo, tier)
                       : { embedUrl: '', type: null };

      if (resolved.embedUrl) {
        // 3a. Render the iframe
        const fallbackNote = resolved.isFallback
          ? `<div style='position:absolute;top:10px;left:10px;
              ...
              ${resolved.servedTier} lesson
              <span style='opacity:.65'>(your ${tier} video coming soon)</span>
             </div>`
          : '';
        ...
        // 3b. No video uploaded yet — show Coming Soon placeholder
        videoArea.innerHTML = `
          ...
          <div style='position:absolute;bottom:18px;left:50%;transform:translateX(-50%);
            ...
            Video coming soon — practise with CBT now
          </div>`;
      }
    }
